...
Defines the API for creating, reading, updating, and deleting records that belong to object definitions. Records store their field values as a `data` object keyed by field definition `apiKey`s, and all record operations are scoped to the caller's organization.
